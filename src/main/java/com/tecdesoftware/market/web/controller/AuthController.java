...
public class AuthController {
    ...
    public ResponseEntity<?> login(@org.springframework.web.bind.annotation.RequestBody LoginRequest request) {
        System.out.println("[AuthController] Petición de login recibida con correo: " + request.getCorreo());
        String token = authService.login(request.getCorreo(), request.getContrasena());
        if (token == null) {
            System.out.println("[AuthController] Credenciales inválidas");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Credenciales inválidas");
        }
        System.out.println("[AuthController] Login exitoso. Token generado.");
